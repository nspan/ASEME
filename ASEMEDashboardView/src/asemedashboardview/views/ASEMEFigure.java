...
public class ASEMEFigure extends RectangleFigure {
	...
	public ASEMEFigure() {
		...
		add(aip2eacFlow = createFlowFigure(true, true));
	...
	private class DashboardLayout extends AbstractLayout {
		...
		//private static final int MAX_BOX_LENGTH = 100;
		
		private static final int MAX_BOX_WIDTH = 150;
		...
		protected LayoutData getLayoutData() {
			...
			Dimension ggSize = ggFigure.getPreferredSize();	//new Dimension(MAX_BOX_LENGTH, MAX_BOX_WIDTH);//ggFigure.getPreferredSize();
			//ggSize.height = Math.min(ggSize.height, MAX_BOX_LENGTH);
			...
			int sagX = data.logoBox.x+ BOX_SPACING/2;
			...
			data.srm2xpdlBox = new Rectangle(data.srmBox.getLeft().x()+ BOX_SPACING/5, data.srmBox.getLeft().y(), srm2iacSize.width, srm2iacSize.height);

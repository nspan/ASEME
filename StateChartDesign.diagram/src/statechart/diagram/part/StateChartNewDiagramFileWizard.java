...
public class StateChartNewDiagramFileWizard extends Wizard {
	...
	*/
	public StateChartNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot,
		...
		myFileCreationPage = new WizardNewFileCreationPage(Messages.StateChartNewDiagramFileWizard_CreationPageName,
				StructuredSelection.EMPTY);
		myFileCreationPage.setTitle(Messages.StateChartNewDiagramFileWizard_CreationPageTitle);
		myFileCreationPage.setDescription(
				NLS.bind(Messages.StateChartNewDiagramFileWizard_CreationPageDescription, ModelEditPart.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage.setFileName(StateChartDiagramEditorUtil.getUniqueFileName(filePath, fileName, "kse")); //$NON-NLS-1$

		diagramRootElementSelectionPage = new DiagramRootElementSelectionPage(
				Messages.StateChartNewDiagramFileWizard_RootSelectionPageName);
		diagramRootElementSelectionPage.setTitle(Messages.StateChartNewDiagramFileWizard_RootSelectionPageTitle);
	...
	*/
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
				...
				Diagram diagram = ViewService.createDiagram(diagramRootElementSelectionPage.getModelElement(),
						ModelEditPart.MODEL_ID, StateChartDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
		...
		try {
			...
			StateChartDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		...
		protected boolean validatePage() {
			...
			boolean result = ViewService.getInstance()
					.provides(new CreateDiagramViewOperation(new EObjectAdapter(getModelElement()),
							ModelEditPart.MODEL_ID, StateChartDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT));
			setErrorMessage(
					result ? null : Messages.StateChartNewDiagramFileWizard_RootSelectionPageInvalidSelectionMessage);
			return result;
		}
	}
}

...
public class StateChartCreationWizard extends Wizard implements INewWizard {
	...
	*/
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(
				StateChartDiagramEditorPlugin.getBundledImageDescriptor("icons/wizban/NewStatechartWizard.gif")); //$NON-NLS-1$
	...
	*/
	public void addPages() {
		diagramModelFilePage = new StateChartCreationWizardPage("DiagramModelFile", getSelection(), "kse"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage.setTitle(Messages.StateChartCreationWizard_DomainModelFilePageTitle);
	...
	public boolean performFinish() {
		IRunnableWithProgress op = new WorkspaceModifyOperation(null) {
			...
			protected void execute(IProgressMonitor monitor) throws CoreException, InterruptedException {
				diagram = StateChartDiagramEditorUtil.createDiagram(diagramModelFilePage.getURI(),
		...
		try {
			...
			if (e.getTargetException() instanceof CoreException) {
				ErrorDialog.openError(getContainer().getShell(), Messages.StateChartCreationWizardCreationError, null,
						((CoreException) e.getTargetException()).getStatus());
			} else {
				StateChartDiagramEditorPlugin.getInstance().logError("Error creating diagram", e.getTargetException()); //$NON-NLS-1$
			}
			return false;
		}
		return diagram != null;

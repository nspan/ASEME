...
public class TransExpr {
	...
	public static boolean HasCondition ( String trExp ) {
		if ( (trExp == null) || ( (!trExp.contains("[")) && (!trExp.contains("]")) ) )
			return false;
		return getNaothConditionExpression(trExp)!=null;
	}
	...
	//thn allaksa gia na pairnw to event opws thelw
	public static String getEventOfExpression(String expression) {
		// pattern for events
		int ctr=0;
		String result="";
		while((ctr<expression.length()) &&((expression.charAt(ctr)!='[') &&(expression.charAt(ctr)!='/')))
		{
			//System.out.println("expression.charAt("+ctr+") = "+expression.charAt(ctr));
			result+=expression.charAt(ctr);
			ctr++;
		}
		if(result.length()<2)
		{
		...
		{
			result=expression;   //mporei na exei mono event
			return result;
		}
	...
	//an exw variables kaleitai auth h synarthsh
	public static String getNaothConditionExpression(String expression,List<Variable> variables)
	{
		String result="";
		return result;
	}
	
	//my Naoth action pattern
	public static String getActionCommand(String expression)
			{
				String res="";
				//String result=new String();
				  Tokenizer myTokenizer = new Tokenizer();
				  //StringTokenizer stT=new StringTokenizer(expression, "[]/");
				  
				  int ctr=0;
				  String conditionExpr=new String();
				  //conditionExpr=myTokenizer.tokenizeTrExp(expression);
				  
				  String actionExpr=new String(getActionOfExpression(expression));
				  //System.out.println("actionExpr "+actionExpr);
				  myTokenizer.add("(^\\s*\\w+(\\.\\w+)*\\s*((\\?)|(\\|\\|)|(!=)|(!)|(=)){1}\\s*(-)?\\w+(\\.\\w+)*\\s*(((\\?)|(\\|\\|)|(!=)|(!)|(=))\\s*)?)+?", 1);		  		 
				  myTokenizer.add("\\(*[0-9]*\\)*",2);
				  //System.out.println("ConditionExpr="+conditionExpr);
					
					try{
						myTokenizer.checkParentheses(actionExpr); //check for open parentheses or trailing operators	      
						myTokenizer.tokenize(actionExpr);
						
					}catch(ParserException e)
					{
						System.out.println(e.getMessage());
					}//kleinei h mesa catch
					
					String boolExp=new String();
				    int sz=myTokenizer.getTokens().size();
				    int prt=0;
				    while(prt<sz)
				    {
				    	boolExp+=myTokenizer.buildExpression(myTokenizer.getTokens().get(prt).sequence);
				    	prt++;
				    }		
				    String st=new String("");//"if("+boolExp+"){\n return true;\n}else{\n return false;\n}";
				   // System.out.println("actionEdited expr "+boolExp);
				    StringTokenizer actionToks = new StringTokenizer(boolExp, "?|");
				    while(actionToks.hasMoreTokens())
				    {
				    	st+=actionToks.nextToken()+";\n";
				    }
				    System.out.println("action command st is "+st);
				return st;
		}
	
	
	//get condition expression for naoth
	//making use of my grammar and my tokenizer
	public static String getNaothConditionExpression(String expression)
	  {		          
		  String result=new String();
		  Tokenizer myTokenizer = new Tokenizer();
		  //StringTokenizer stT=new StringTokenizer(expression, "[]/");
		  ...
		  myTokenizer.add("(^\\(*\\s*\\w+(\\.\\w+)*\\s*((&&)|(\\|\\|)|(!=)|(!)|(==)|(>)|(<)|(>=)|(<=)|(\\+)){1}\\s*(-)?\\w+(\\.\\w+)*\\s*\\)*\\s*(((&&)|(\\|\\|)|(!=)|(!)|(==)|(>)|(<)|(>=)|(<=)|(\\+))\\s*)?)+?", 1);		  		 
		  myTokenizer.add("\\(*[0-9]*\\)*",2);
		 // myTokenizer.add("([0-9]+\\.[0-9]+)*",3);
		  //tokenizer.add("[0-9]+",6); // integer number
		  //System.out.println("ConditionExpr="+conditionExpr);
			
			try{
				myTokenizer.checkParentheses(conditionExpr); //check for open parentheses or trailing operators	      
				myTokenizer.tokenize(conditionExpr);
		    ...
		    String st="if("+boolExp+"){\n return true;\n}else{\n return false;\n}";
			return st;//"return ("+boolExp+");";
	  }                 
	
	//function only to be used for printing the initial message of condition
	public static String getNaothInitCondition(String expression)
	{
		return expression;
	}
	
	//thn allaksa gia na vgazei ta dika mou sxolia
	...
	public static String getOldCondition(String expression)
	{
		Pattern conditionPattern = Pattern
				.compile("^([\\w\\W&&[^/\\[\\]]]+)?(\\[[\\w\\W&&[^\\[\\]]]+\\])(/[\\w\\W]+)?$");
		Matcher conditionMatcher = conditionPattern.matcher(expression);
	...
	//template gia thn eval ths condition sto Naoth
	public static String EvalNaothCondExpr(String expr)
	{
